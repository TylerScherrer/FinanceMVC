...
public async Task<IActionResult> AssignTaskToTime(int taskId, int hour, DateTime date)
{
    Console.WriteLine($"AssignTaskToTime called with TaskId: {taskId}, Hour: {hour}, Date: {date.ToShortDateString()}");
    ...
    if (taskId <= 0 || hour < 0 || hour > 23)
    {
        ...
        return RedirectToAction("Index", "Planner", new { date });
    }

    try
    {
        // Log task assignment attempt
        Console.WriteLine($"Attempting to assign TaskId: {taskId} to Hour: {hour} on Date: {date.ToShortDateString()}");

        // Attempt to assign the task
        await _toDoService.AssignTaskToTimeAsync(taskId, hour, date);

        // Success feedback
        Console.WriteLine($"TaskId: {taskId} successfully assigned to Hour: {hour} on Date: {date.ToShortDateString()}");
        TempData["SuccessMessage"] = "Task successfully assigned to the selected time.";
    }
    catch (Exception ex)
    {
        // Log the exception details
        Console.WriteLine($"Error occurred while assigning TaskId: {taskId} to Hour: {hour} on Date: {date.ToShortDateString()}. Exception: {ex.Message}");
